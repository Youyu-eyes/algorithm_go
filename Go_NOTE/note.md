...
| 功能 | 代码 |
| --- | --- |
| 第一个 $\ge v$ 的数 | `sort.SearchInts(s, v)` |
| 第一个 $> v$ 的数 | `sort.SearchInts(s, v + 1)` |
...
`nums := slices.Clone(arr)` 表示对 $arr$ 切片进行克隆
`nums := slices.Compact(arr)` 表示对 $arr$ 切片进行去重，注意，这里的去重是相邻项去重，不相邻但相等的数保留，如
$$
1\ 2\ 2\ 3\ 3\ 4\ 2\ 1\ 3\ 3\ 3
$$
